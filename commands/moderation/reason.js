exports.run = async(client, message, args) => {
    client.utils.permissionCheck(message, `MANAGE_MESSAGES`).then(bool => {
        ...
        client.db.findById(client.user.id, function(err, res) {
            ...
            if ((res.cases[args[0]].moderator !== message.author.id) && (!message.member.hasPermission(`ADMINISTRATOR`))) return message.reply(`You are not allowed to edit cases that you didn't create.`).then(a => a.delete({ timeout: 10000 }));
            let reason = args.slice(1).join(` `)
            if (!reason) return client.utils.missing(message, `NEW REASON`);
            ...
            if (!channel) {
                return client.db.findByIdAndUpdate(client.user.id, {
                    cases: res.cases
                }).then(() => {});
            } else {
