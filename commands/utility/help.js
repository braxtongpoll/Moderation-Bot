...
async function callMenu(client, message, res) {
    ...
    fs.readdirSync(path.join(__dirname, `../`)).forEach(dir => {
        ...
        for (let file of cmds) {
            ...
            if (client.config.aliases[info.info.name]) {
                aliases = client.config.aliases[info.info.name].join(" ") || "No Aliases";
            } else {
                console.log(info.info.name + " Missing from config.aliases");
            };
            if (commandData[dir]) {
    ...
    embed.setDescription(client.config.helpMenuMessage);
    embed.setFooter(`Viewing page 0 of ${pages.length}`)
